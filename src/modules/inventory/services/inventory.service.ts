...
export class InventoryService extends BaseService<any> {
    ...
    async findAllIssue(query: PaginationQueryDto) {
        const where: any = { type: PurchaseOrderType.OUT, deletedAt: null };
        if (query.q) {
            where.orderCode = { contains: query.q };
        }
        return this.paginate(
    ...
    async createIssue(userId: number, dto: CreateIssueDto) {
        return this.prisma.$transaction(async (tx) => {
            ...
            for (const item of dto.items) {
                await tx.transferItem.create({
                    data: {
                        transferType: TransferType.PURCHASE, // Using Purchase for simple export log
                        recordId: order.id,
                        productId: item.productId,
                        quantity: item.quantity,
                        price: item.price,
